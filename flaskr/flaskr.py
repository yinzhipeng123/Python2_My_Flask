...
from test.blue_read import blue_read
...
app.register_blueprint(blue_read)
...
def add_entry():
    if session.get('logged_in'):
        ...
        g.db.execute('insert into entries (id,title, text,author) values (?,?,?,?)',
                     [None, request.form['title'], request.form['text'], userid])
        g.db.commit()
...
def register():
    if request.method == 'POST':
        ...
        if dbcur:
            ...
            g.db.execute('insert into users (id,user, password) values (?,?,?)',
                         [None, request.form['username'], request.form['password']])
            g.db.commit()
            flash('注册成功')
